  ...
  return (
    <div className="p-6 md:p-8 max-w-[1280px]">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[28px] font-bold tracking-tight text-[#1D1D1F]">Dashboard</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <Tile label="Orders Today" value={kpi.orders_today} subtitle="since midnight" />
        <Tile label="30-Day Total" value={kpi.orders_30d.toLocaleString()} subtitle="orders" />
        <Tile label="Pending" value={kpi.pending_orders} subtitle="needs action" accent="amber" />
        <Tile label="Products" value={kpi.total_products} subtitle="SKUs" accent="green" />
        <Tile label="Integrations" value={kpi.active_integrations} subtitle="connected" accent="blue" />
      </div>

      {/* Per-Warehouse Breakdown */}
      {selectedWh === "all" && kpi.warehouse_breakdown.length > 1 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-semibold text-[#1D1D1F] mb-3">Warehouse Breakdown</h2>
            ...
            {kpi.warehouse_breakdown.map((w) => (
              <div key={w.id} className="bg-white rounded-2xl p-5 shadow-sm border border-black/5 hover:shadow-md transition-shadow cursor-pointer" onClick={() => handleWhChange(w.id)}>
                ...
                <div className="flex gap-4">
                  ...
                  <div><p className="text-[22px] font-bold text-[#34C759]">{w.shipped}</p><p className="text-[10px] text-[#86868B] uppercase">Shipped</p></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden">
        ...
        <table className="w-full">
          <thead><tr className="text-left text-xs font-medium text-[#86868B] border-b border-black/5">
            ...
            <th className="px-5 py-3">Source</th><th className="px-5 py-3">Status</th>
