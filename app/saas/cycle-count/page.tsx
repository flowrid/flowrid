...
export default function CycleCountPage() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1280px]">
      <h1 className="text-[28px] font-bold tracking-tight text-[#1D1D1F] mb-6">{t("cycleCount")}</h1>
      ...
      {selectedSession ? (
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
              {selectedSession.status === "created" && (
                <button onClick={() => updateSessionStatus("in_progress")}
                  className="bg-[#007AFF] text-white px-4 py-2 rounded-full text-sm font-semibold">{t("startCount")}</button>
              )}
              {selectedSession.status === "in_progress" && (
                <button onClick={() => updateSessionStatus("completed")}
                  className="bg-[#34C759] text-white px-4 py-2 rounded-full text-sm font-semibold">{t("completeCount")}</button>
              )}
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden">
            <table className="w-full">
              <thead><tr className="text-left text-xs text-[#86868B] border-b"><th className="px-5 py-2.5">{t("product")}</th><th className="px-5 py-2.5">{t("location")}</th><th className="px-5 py-2.5 text-right">{t("expected")}</th><th className="px-5 py-2.5 text-right">{t("counted")}</th><th className="px-5 py-2.5 text-right">{t("variance")}</th><th className="px-5 py-2.5">{t("statusCol")}</th></tr></thead>
        ...
        <>
          ...
          <div className="mb-6 bg-white rounded-2xl shadow-sm border border-black/5 p-6">
            <h2 className="text-[15px] font-semibold text-[#1D1D1F] mb-4">{t("newCycleCount")}</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-[11px] font-medium text-[#86868B] uppercase mb-1">{t("nameLabel")}</label>
                  <input type="text" value={sessionName} onChange={(e) => setSessionName(e.target.value)} placeholder={t("autoGenerated")}
                    className="w-full bg-[#F5F5F7] border-0 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#ed6d00]/20" />
                </div>
                <div>
                  ...
                  <select value={countType} onChange={(e) => setCountType(e.target.value)}
                    className="w-full bg-[#F5F5F7] border-0 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#ed6d00]/20">
                    <option value="full">{t("fullCount")}</option>
                    <option value="zone">{t("byZone")}</option>
                    <option value="abc">{t("abcAnalysis")}</option>
                    <option value="random">{t("randomSample")}</option>
                  </select>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden">
            <table className="w-full">
              <thead><tr className="text-left text-xs text-[#86868B] border-b"><th className="px-5 py-3.5">{t("nameLabel")}</th><th className="px-5 py-3.5">{t("type")}</th><th className="px-5 py-3.5">{t("statusCol")}</th><th className="px-5 py-3.5">{t("created")}</th></tr></thead>
              <tbody className="divide-y divide-black/[0.04]">
                {sessions.map((s: any) => (
                ...
                {sessions.length === 0 && (
                  <tr><td colSpan={4} className="px-5 py-12 text-center text-[#86868B] text-sm">{t("noCycleCounts")}</td></tr>
                )}
              </tbody>
            </table>
          </div>
