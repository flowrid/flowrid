...
const RELATED_3PLS = [
  { name: "ShipBob", state: "IL", category: "apparel" },
...
export default async function TrackingVisibilityPage() {
  ...
  return (
    <div>
      ...
      {/* Related 3PL Cross-Sell */}
      <section className="mb-12">
        <h2 className="text-xl font-bold text-text mb-2">
          {t("tools.tracking.crossSell.title")}
        </h2>
        <p className="text-sm text-text-secondary mb-6 leading-relaxed max-w-[600px]">
          {t("tools.tracking.crossSell.description")}
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {RELATED_3PLS.map((pl) => (
            <Link
              key={pl.name}
              href={`/3pl/${pl.state}/${pl.category}`}
          ...
          >
            ...
            <h3 className="text-base font-bold text-text group-hover:text-primary transition-colors">
              {t("tools.tracking.crossSell.browseAll")}
            </h3>
            <p className="text-sm text-text-secondary mt-1">
              {t("tools.tracking.crossSell.browseAllDesc")}
            </p>
          </Link>
        </div>
      </section>
